...
export function Detailpageaction() {
  const { id } = useParams();
  const navigate = useNavigate();
  const archive_title = getNote(id).archived === true ? "Aktifkan" : "Arsipkan"

  const onArchiveHandler = (noteId) => {
      getNote(id).archived === false ? archiveNote(id) : unarchiveNote(id);
      navigate('/');
  };
  ...
  return (
    <div className="detail-page__action">
      <button className="action" type="button" title={archive_title} onClick={() => onArchiveHandler(id)}>
